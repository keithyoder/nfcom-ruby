  ...
  module Webservices
    ...
    class Autorizacao < Base
      ...
      def enviar(xml_assinado)
        ...
        # Usar ResponseParser para processar a resposta
        # decompressed_doc = Utils::ResponseDecompressor.extract_and_decompress(response.body)
        Nfcom::Parsers::ResponseParser.new(response).parse_autorizacao
      ...
      def validate_http_response!(response)
        ...
        raise Errors::SefazError, "Erro HTTP #{response.code}: #{response.message}"
      end
